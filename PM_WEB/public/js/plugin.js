...
/**$.fn.backTop = function (settings) {
    ...
    /**------列表xDataTable的封装------*/
    $.fn.xDataTable = function (settings) {
        var el = $(this);
        if (el.length == 0) return;
        var defaultSettings = {
            "responsive": true,
        ...
        var options = $.extend(true, defaultSettings, settings);
        if (options.operateColumn != "" && options.operateColumn != null) {
            options.editorColumn.defaultContent=options.operateContent;
            options.columns.push(options.editorColumn);
        }
    ...
    $.fn.xDialog = function (settings) {
        ...
        var defaultSettings = {
            ...
            hide:function(){},
            validator:true,
            //selectTarget:"#ProjectProvinceName,#ProjectCityName,#CurrentProjectProgressState",
            clearTarget:"#config-form"
        }
        var options = $.extend(true, defaultSettings, settings);
        el.on('shown.bs.modal',function(){
            if(options.selectTarget)
            {
                $(options.selectTarget).selectpicker().on('changed.bs.select', function (e) {
                    var validatorinputId=this.id+"_validator";
                    var val=e.target.value==-1?"":e.target.value;
                    $("#"+validatorinputId).val(val);
                    $(options.clearTarget).
                        data('bootstrapValidator').updateStatus(this.id, 'NOT_VALIDATED').validateField(this.id);
                });
            }
            options.show();
        });
        el.on('hide.bs.modal',function(){
            $(options.clearTarget+" input,"+options.clearTarget+" textarea").val("");
            var select=$(options.clearTarget+" select");
            if(select.length!=0) {
                select.val("-1");
                select.selectpicker('refresh');
            }
            if(options.validator) {
                $(options.clearTarget).data('bootstrapValidator').resetForm();
            }
            options.hide();
        });
        this.show=function(){
    ...
    /**------验证的封装------*/
    $.fn.xValidator=function (settings) {
        var el = $(this);
        if (el.length == 0) return;
        var defaultSettings = {
            excluded: [':disabled'],
    ...
    $.fn.xLayer = function (settings) {
        ...
        if(typeof(layer)!="undefined") {
            ...
            //询问框
            this.confirm = function (text, onOk, onCancel) {
                layer.confirm(text, onOk, onCancel);
            }
            //提示层
    ...
    /**------时间控件的封装------*/
    $.fn.xdatepicker = function (settings) {
        var el = $(this);
        if (el.length == 0) return;
        var defaultSettings = {
            autoclose: true,
        ...
        el.datepicker(options).on('changeDate', function(e) {
           if(options.validator)
           {
               ...
               $(options.clearTarget).
                   data('bootstrapValidator').updateStatus(name, 'NOT_VALIDATED').validateField(name);
    ...
    /**---ajax封装-------
     例： $.fn.ajaxHandler({url: '/Dept/SaveModel'}).done(function (data) {}).fail(function (res) { });
     ---------------*/
    $.fn.ajaxHandler = function (settings) {
        var defaultSettings = {
            dataType: 'json',
            type: 'post',
            async: true,
        ...
        return $.ajax(options).then(function (data) {
            if (data.Success||typeof(data.Success)=="undefined") {
                ...
                if(options.layer) $.fn.xLayer().msg(data.Message);
    ...
    $.fn.areaSelect=function(settings){
        var defaultSettings = {
            ProvinceId: "ProjectProvinceName",
            CityId:"ProjectCityName",
            AreaId:"ProjectAreaName",
            validator:true,
            selectTarget:"#ProjectProvinceName,#ProjectCityName",
            clearTarget:"#config-form",
            CityCallBack:function(){},
            AreaCallBack:function(){}
        }
        var options = $.extend(true, defaultSettings, settings);
        var ProvinceEl=$("#"+options.ProvinceId),
            CityEl=$("#"+options.CityId),
            AreaEl=$("#"+options.AreaId);
        var fnarea={
            options:options,
            initRefresh:function()
            {
                ProvinceEl.get(0).selectedIndex = 0;
                CityEl.get(0).selectedIndex = 0;
                AreaEl.get(0).selectedIndex = 0;
                ProvinceEl.selectpicker('refresh');
                CityEl.selectpicker('refresh');
                AreaEl.selectpicker('refresh').on('changed.bs.select', function (e) {
                    var validatorinputId=this.id+"_validator";
                    var areatext=$(e.target).find("option:selected").text();
                    if(e.target.value==-1) $("#"+validatorinputId).val("");
                    else  $("#"+validatorinputId).val(areatext);
                    var protext=ProvinceEl.find("option:selected").text();
                    var citytext = CityEl.find("option:selected").text();
                    fnarea.options.AreaCallBack(protext,citytext,areatext);
                });

            },
            clearSelect:function(el){
                var validatorinputId=el.attr("id")+"_validator";
                $("#"+validatorinputId).val("");
                el.empty();
                el.append("<option value='-1'>----请选择----</option>");
                el.selectpicker('refresh');
            },
            initSelect:function(el,indexArry){
                var validatorinputId=el.attr("id")+"_validator";
                $("#"+validatorinputId).val("");
                el.empty();
                el.append("<option value='-1'>----请选择----</option>");
                var index=indexArry[0],subindex=indexArry[1];
                var data=index==-1?administrative:administrative[index].children;
                data=subindex==-1?data:data[subindex].children;
                $.each(data, function (i, n) {
                    var node = "<option value='" + i + "'>" + n.name + "</option>";
                    el.append(node);
                });
                el.selectpicker('refresh');
                return data;
            ...
            initProvinceSelect:function(){
                fnarea.initSelect(ProvinceEl,[-1,-1]);
                ProvinceEl.selectpicker('refresh').on('changed.bs.select', function (e) {
                    fnarea.initSelect(CityEl,[e.target.value,-1]);
                    fnarea.clearSelect(AreaEl);
                });
                CityEl.selectpicker('refresh').on('changed.bs.select', function (e) {
                    var Provinceindex=ProvinceEl.selectpicker('val');
                    var indexArry=[Provinceindex,e.target.value];
                    var areadata=fnarea.initSelect(AreaEl,indexArry);
                    var protext=ProvinceEl.find("option:selected").text();
                    var citytext = $(e.target).find("option:selected").text();
                    fnarea.options.CityCallBack(protext,citytext,areadata);
                });
            },
            setSelectText:function(el,text)
            {
                var selecter=$("option:contains('"+text+"')",el);
                if(selecter.length==1) {
                    selecter.attr('selected', true);
                    el.selectpicker('refresh');
                    var index=selecter.val();
                    return index;
                }
                return -1;
            },
            validator:function(){
                $(options.selectTarget).selectpicker().on('changed.bs.select', function (e) {
                    var validatorinputId=this.id+"_validator";
                    if(e.target.value==-1) $("#"+validatorinputId).val("");
                    else  $("#"+validatorinputId).val($(e.target).find("option:selected").text());
                    $(options.clearTarget).
                        data('bootstrapValidator').updateStatus(this.id, 'NOT_VALIDATED').validateField(this.id);
                });
            }
        }
        this.disbaleSelect=function(disbale)
        {
            var Els=$("#"+options.ProvinceId+",#"+options.CityId+",#"+options.AreaId);
            Els.attr("disabled",disbale);
        }
        this.setSelect=function(Province,City,Area)
        {
            var index=fnarea.setSelectText(ProvinceEl,Province);
            fnarea.initSelect(CityEl,[index,-1]);
            var subindex=fnarea.setSelectText(CityEl,City);
            fnarea.initSelect(AreaEl,[index,subindex]);
            fnarea.setSelectText(AreaEl,Area);
        }

        fnarea.initRefresh();
        fnarea.initProvinceSelect();
        if(options.validator) fnarea.validator();
        return this;
    }

    /**------ie版本控制的封装------*/
    /**var isIE=$.fn.isIE(10, 'lt');*/
    $.fn.isIE = function (ver, c) {
        var browser = {};
        function test(ver, c) {
    ...
    $.fn.iframeLoad = function (settings) {
